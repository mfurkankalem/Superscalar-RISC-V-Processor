A superscalar RISC-V processor implemented in SystemVerilog, verified with Verilator. The name stands for **i**n-order fetch/decode, **o**ut-of-order **i**ssue: instructions are fetched and decoded in program order, but issue one per cycle into one of two independent execution paths (ALU or MEM). Once issued, an ALU-path instruction and a MEM-path instruction can execute concurrently, so instructions may complete out of order — a reorder buffer (ROB) then retires them strictly in order. The design fetches instructions from a hex memory image, executes the RV32I instruction set, and logs its execution trace for verification against a reference model.
